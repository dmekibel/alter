...
- ✅ **personal-battery-physics (v898, Fable session, DONE):** implemented as THE DAY'S VOLTAGE — the geometry (time axis) stayed inviolable (a rate-modulated bar height would desync from the hour grid = the v488/v496 bug class; earns untouched per law 7). `energyRate(k)` reads which of the user's OWN P.ingredients are evidenced in day k's logs minus drift drag → clamped 0.8–1.2 → `voltClass()` paint-only filter tiers (volt-hi/lo) on charge elements only (chgseg/matchseg/onplan/live; never ghost/drift/future). Un-profiled = zero visual change. Found+fixed a float-dust boundary bug (0.9000…01 skipping volt-lo) in-session. Verified: tiers fire, invariants hold, grid alignment intact, preship logic invariants pass. Gesture feel DEVICE-UNTESTED.
