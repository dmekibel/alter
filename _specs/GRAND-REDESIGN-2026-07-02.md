...
### §12 RUN STATUS (2026-07-03, v815–v819)
**THE TRUE CANON = the 7 images DAVID UPLOADED (2026-07-02 20:23) → `_specs/_frames/_uploads/` — compare against THESE, not the chat-widget renders** (also recovered, `_specs/_frames/`, useful as secondary). Built + preview-compared at 390px: **THE KIT** (k-sec/k-q/k-sub/k-card/k-dur + choice-row v3 as `.k-row`/`.rc-row` vars) · **① cockpit idle** (compact hero, hard shadows, full-screen, chrome gone — v815) · **② toolbox one-door** (bare header + circle-X, 19px hero card no-glow, green GO, 54px daily circles w/ done=green-ring inversion, ghost Close — v816) · **⑦ reflection cards** (own-color outline/tint/ignition rows — v817) · **④ quest card** (foil-edge-as-progress, icon well, circle ladder, quiet gold sheen, pink session door — v818) · **⑤ INTRO to upload 11** (clean wide-tracked wordmark, bare-word secondaries, iconless pink slab, sky dots, ring-orbit glint — v819) · **⑥ ONBOARDING to upload 12** (full-bleed night, gold progress dashes, guardian circle, vibe beat «Что сейчас ближе к правде?» = 4 element v3 rows w/ auto-advance; room → task beat; 13 dict entries — v819). REMAINING: **③ edge inspector** (upload 09 — new surface, needs David's device loop), quest-card garnish (sub-line/pips/mint strip, tiered bounties → economy run), the NAV QUESTION (on-device decision), motion polish (the motion run). EN gaps logged: decomposeGoal starter steps + the "1 active" chip speak English in RU mode (pre-existing, → B-sweep).

## 12 · THE FRAME-FIDELITY RUN (the gameplan David asked for — the 7 chat frames ARE the pixel spec)
**Root cause of "I don't see it" (v804-814):** functional equivalents shipped in the app's OLD skins — same features, old geometry. The frames have a different body: bigger type, fatter radii, more air, exact component shapes. This run rebuilds bodies, not behaviors.
**Step 1 — THE KIT (one CSS component set, then reused everywhere):** night card (radius 16, ink 2.5px, 0-4px-0 shadow, #22091a/#1e0b18 surfaces) · type scale from the frames (21px question titles, 15.5px row labels, 12px subs, 11.5px letterspaced gold/pink section labels) · THE DOORS (54px, solid/striped/ghost) · duration chips · foil progress frame (card moments) · icon wells ONLY where frames show them (quest card) · **THE FINAL CHOICE-ROW (locked by David from the two frames):** unchosen = GHOST (dark tinted fill + the answer's colored DASHED outline + bare colored icon + white label) · chosen = IGNITION into its own color's full 9px stripes + hairline gold inset + bare check · no circles/squares/badges ever.
...
- **CHOICE-ROW v3 (final-final):** NO gold on selection — the ignition (ghost → own-color stripes + ink border + bare check) IS the selection. Gold is hereby reserved for EARNED moments only (matched blocks, crowns, mints, ranks) — selection ≠ reward, and the scarcity keeps gold meaning something.
