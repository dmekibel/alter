...
## v2 addendum (same day — David: Pokémon holo, metallic, fire, "even better animations")
**The RARITY LADDER (finishes):** матовый → металл (silver sheen sweep) → фольга (gold foil sweep) → голо (rainbow holo sheen + hue-shift) → сияющий (shiny: holo + blue pulse + pop). CSS-only — the .foil/foilSweep pattern already in the app generalizes to all of these. Finishes map onto badges (rank), goal cards, the trainer card, and tier-5 mints.
**STREAK FIRE LEVELS (evolves the established streak):** 1–2 Уголёк (warm dot) · 3–6 Огонь (flicker) · 7–20 Пламя (flicker+glow ring) · 21+ Синее пламя (blue-white, the hottest). Applies to the global streak chip AND per-habit rows.
**BADGES = COLLECTIBLE CARDS** in a binder (game pane): earned cards keep their finish, unearned slots visible as aspiration, next-badge progress line always shown. Landmark mints do the card-mint animation (stamp + sheen sweep). Onboarding mints the TRAINER CARD (identity word) — matte, upgrades finishes as the profile deepens.
**SHINY VARIANTS (ethical variable reward):** ~1/20 tier-2 ignitions roll shiny (bonus spark + the shiny finish moment). LAW: a shiny is a bonus ON TOP of an already-earned reward — never a withheld base reward, never slot-machine odds on the outcome itself. Pokémon model, not casino model.
**Menus round 2 (mockups in chat):** edge inspector (right-edge in-place block editor — retires full-screen blockEdit for small edits) · badge binder · goals=quest cards w/ WOOP card-back · habits carry per-habit fire · month view = crown calendar (matte/shining/crowned/fire heat-map) · day-tools ⋯ = icon grid · You tab topped by the trainer card.

## v3 (David: "super boring — more interesting vibes, Pokémon colors")
The polite-sheen ladder is DEAD for card moments. **ELEMENT-TYPED CARD TREATMENTS** (Pokémon energy types), one per badge family: Вернулся=ПРИЗМА (full-spectrum interference bands, infinite hue-shift, gold-prism frame) · Огонь=ВСПЫШКА (rotating burst rays + rising embers + fire aura) · Глубина=КОСМОС (space foil, twinkling stars, nebulae, ice-blue frame) · Смелость=crimson slash · Точность=gold lattice · Садовник=bioluminescent green. Every card TILTS in 3D (on device: gyroscope-driven — tilt the phone, the light moves; the show-your-friends moment). Cards carry FLAVOR TEXT = the user's own story of the earn. Rarity tags: РЕДКИЙ/ЭПИК/ЛЕГЕНДА gradient pills.
SCOPE LAW: this grade (blend modes, conic rays, 3D) = CARD MOMENTS ONLY (mint, binder, trainer card) — ambient UI keeps the light sheen; the contrast IS the reward. Chat mockup approved direction 2026-07-02 (three full cards: prism/burst/cosmos).
