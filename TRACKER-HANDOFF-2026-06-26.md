# ALTER — handoff · live **v889** (2026-07-04; older bodies below are historical)

## ⚡ 2026-07-04 (later) — THE DEPTH BUILD greenlit · spec complete
**Read `_specs/SPEC-DEPTH-BUILD-2026-07-04.md` first in the next build session.** The depth marathon (course pedagogy digest + soul digest + fieldguide mining: 4 files in `_mined/`, LOCAL-only) produced the full system: 7 new constitution laws (one-skill thesis · gauge=threshold-gate for the 4 voices · mantra law · chores first-class · wide-day silence · emotional-ladder comebacks · no-NPC-treadmill gamification), a 41-card theory/proverb deck, 3 build waves. **Wave 1 (next session, Opus):** onboarding delta (+2 GLANCE questions) · PM CLOSE v2 (win/learn/dayRating-with-Rebound-parity/WOOP-chips/INSTALL mantra beat/deck deal) · Reset sprint + chore chains · moment-listener v1 (5 triggers → ONE tool) · TLM pings with the user's year-words. Zero timeline paint in Waves 1-2. SCHEMA bump required. Every deck string EN+RU same commit. Also shipped this session: v885-v889 (points chip + gem flash, solid-piece onboarding, back button, moment-based awards, first-seconds tracking fix).

## ⚡ 2026-07-04 — ONBOARDING V3 (v884) — chat-proposal marathon → one-pass build
**Spec = `_specs/ONBOARDING-V3-LOCKED-2026-07-04.md`** (every piece David-picked from live chat mockups). Shipped in v884, preview-verified end-to-end in RU, zero console errors: Spark-script cold open (word-by-word materialize, hand-crafted RU) · ONE twilight room + per-section aurora (orange room retired) · toggleable answers + sleeping/igniting Next (auto-advance removed) · glowing spark-confetti per answer (star+gems+streak → battery cell) · per-section guardian ECHO · breath: sparks gather on the inhale, ONE companion star born per exhale · constellation-of-answers («ЭТО ТЫ») · speaking-flames register question (→ `P.register` soft/straight/push; consume it in guide/PM/notifications copy — NOT yet consumed anywhere) · thumb-seal pact ring (**hold gesture = DEVICE-UNTESTED**) · holo mint card (wants chips + rainbow strip + tilt) · shooting-star send-off with «завтра в 8:00/9:00» · peak question CUT (P.peak defaults stay guarded).
**New laws (memory): NO-NEON** (no glowing thin outlines ever; glow only halos SOLID ink-edged pieces) · **guardian = the real app icon** (ss-mark SVG; never a stand-in). **Ledgered:** flame-dial as a settings re-tune surface · "the card ages" (re-stamp day 7/30) · answer-memory on revisit · skip-runner hold-to-fast-forward (intro tap-to-reveal built).
**NEXT:** ① David's device pass of the whole V3 flow (re-run onboarding from You-tab). ② Day-1 routing matrix from the V2 paper §2 (Full/Light/Relief/Night-Seed off vibe × openBand) — now should also cite `P.register` + pactDays. ③ Wire `P.register` into the guardian's actual copy surfaces.
