...
(function () {
  ...
  function onboard() {
    ...
    function finish() {
      ...
      S.guide.mode = "guided";
      var lvl = data.level === "steady" ? 4 : data.level === "some" ? 2 : 0;
    ...
    function draw() {
      ...
      if (step === 6) { add(body, "div", "ob-q", "Who are you becoming?"); add(body, "div", "ob-sb", "pick a couple — present tense, the you you're growing into · type your own"); var gr = add(body, "div", "ob-row"); IDENT_SEED.forEach(function (id) { var on = data.identity.indexOf(id) >= 0, c = chip(gr, '<i class="ti ti-user-star"></i> ' + esc(id) + (on ? ' ✓' : ''), on, "#9a5cf0", "#241548"); c.onclick = function () { var i = data.identity.indexOf(id); if (i >= 0) data.identity.splice(i, 1); else data.identity.push(id); draw(); }; }); data.customIdent.forEach(function (v) { var on = data.identity.indexOf(v) >= 0, c = chip(gr, '<i class="ti ti-user-star"></i> ' + esc(v) + (on ? ' ✓' : ''), on, "#ff8a3a", "#4a2400"); c.onclick = function () { var i = data.identity.indexOf(v); if (i >= 0) data.identity.splice(i, 1); else data.identity.push(v); draw(); }; }); typeRow("the you you're becoming…", function (v) { data.customIdent.push(v); data.identity.push(v); });
        add(body, "div", "ob-lbl", "A STRENGTH TO LEAN ON"); var vr = add(body, "div", "ob-row"); VIRTUES.forEach(function (v) { var on = data.virtue === v.k, c = chip(vr, v.e + ' ' + v.l, on, v.c, "#160510"); c.onclick = function () { data.virtue = on ? "" : v.k; draw(); }; });
        add(body, "div", "ob-lbl", "YOUR ONE THING TODAY"); var orow = add(body, "div", "ob-addrow"); var oinp = document.createElement("input"); oinp.className = "ob-input"; oinp.placeholder = "the one move that'd make today count…"; oinp.value = data.oneThing; orow.appendChild(oinp); oinp.addEventListener("input", function () { data.oneThing = oinp.value; }); }
      if (step === 7) { add(body, "div", "ob-q", "Your daily habits"); add(body, "div", "ob-sb", "pick the small basics you want to keep showing up for · type your own"); if (!data._habInit) { data._habInit = true; DEFAULT_HABITS.forEach(function (h) { data.habitsSel[h.id] = true; }); } var gr = add(body, "div", "ob-row"); DEFAULT_HABITS.concat(EXTRA_HABITS).forEach(function (h) { var on = data.habitsSel[h.id], c = chip(gr, h.e + ' ' + h.l + (on ? ' ✓' : ''), on, h.color, "#160510"); c.onclick = function () { data.habitsSel[h.id] = !data.habitsSel[h.id]; draw(); }; }); data.customHabits.forEach(function (h) { var on = data.habitsSel[h.id], c = chip(gr, h.e + ' ' + esc(h.l) + (on ? ' ✓' : ''), on, "#ff8a3a", "#4a2400"); c.onclick = function () { data.habitsSel[h.id] = !data.habitsSel[h.id]; draw(); }; }); typeRow("a habit of your own…", function (v) { var id = "h" + Date.now().toString(36); data.customHabits.push({ id: id, e: "✨", l: v, type: "build", per: 0, color: "#9a5cf0" }); data.habitsSel[id] = true; }); }
      if (step === 8) { add(body, "div", "ob-q", "Where are you starting?"); add(body, "div", "ob-sb", "no wrong answer — it just sets where your journey opens"); var col = add(body, "div", "ob-col"); [["new", "ti-seedling", "Just starting", "I want to build the basics", "#23c98a", "#06321f"], ["some", "ti-flame", "Finding my rhythm", "I keep some days, drop others", "#ff8a1e", "#3a1d00"], ["steady", "ti-mountain", "Pretty steady", "I mostly follow through", "#3a9ae6", "#0a2540"]].forEach(function (o) { var on = data.level === o[0], c = add(col, "span", "ob-ch" + (on ? " on" : "")); c.style.background = on ? o[4] : mixDark(o[4]); c.style.color = on ? o[5] : o[4]; c.style.textAlign = "left"; c.innerHTML = '<i class="ti ' + o[1] + '"></i> <b>' + o[2] + '</b><br><small style="opacity:.85">' + o[3] + '</small>'; c.onclick = function () { data.level = o[0]; draw(); }; }); }
  ...
  function init() {
    ...
    if (S.profile && S.profile.set) { try { if ((S.guide || {}).mode === "guided") journeyTick(); } catch (e) {} setTimeout(function () { try { openJourney(); } catch (e) {} }, 200); } // JOURNEY IS HOME (David 2026-06-28): land on the journey path on every cold open
