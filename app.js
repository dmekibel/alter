...
(function () {
  ...
  function blockEdit(b, k) {
    var B = el("sheetBody"); B.innerHTML = ""; openSheet(); pushUndo(); add(B, "div", "sttl", "Edit block");
  ...
  function logEdit(e, k) { // tapping a REAL-lane bubble opens the SAME kind of menu as a plan bubble — type, duration, start, priority (David 2026-06-25)
    var B = el("sheetBody"); B.innerHTML = ""; openSheet(); pushUndo(); add(B, "div", "sttl", "Edit activity");
    ...
    add(B, "div", "lbl", "type — what this activity is"); var swb = add(B, "button", "add2"); swb.style.margin = "4px 0"; swb.innerHTML = '<i class="ti ' + tiClass(e) + '"></i> ' + esc(e.title) + ' — switch'; swb.onclick = function () { bentoPicker({ title: "Switch to…", onPick: function (x) { e.title = x.title; e.catK = x.catK || null; e.color = x.color || e.color; if (x.domain) e.domain = x.domain; tx.value = x.title; reflowLogs(k); save(); renderToday(); swb.innerHTML = '<i class="ti ' + tiClass(e) + '"></i> ' + esc(e.title) + ' — switch'; } }); };
    function _dlbl(m) { return m < 60 ? m + "m" : (m % 60 ? (Math.floor(m / 60) + "h " + (m % 60) + "m") : (m / 60 + "h")); }
    add(B, "div", "lbl", "duration — tap ± to extend (works on a tiny sliver too)"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setD((e.mins || 15) - 5); }; var vv = add(w, "span", "stpv", _dlbl(e.mins || 15)); add(w, "button", "stp", "+").onclick = function () { setD((e.mins || 15) + 5); }; function setD(v) { e.mins = Math.max(5, Math.min(720, v)); vv.textContent = _dlbl(e.mins); reflowLogs(k); save(); renderToday(); } })();
    add(B, "div", "lbl", "start"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setS(hm(e.time) - 5); }; var vv = add(w, "span", "stpv", fmt(hm(e.time))); add(w, "button", "stp", "+").onclick = function () { setS(hm(e.time) + 5); }; function setS(v) { v = Math.max(0, Math.min(1740, v)); e.time = pad(Math.floor(v / 60)) + ":" + pad(v % 60); reflowLogs(k); save(); renderToday(); vv.textContent = fmt(hm(e.time)); tm.value = e.time; } })();
