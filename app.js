  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (status === "ok" && !partial) { // DONE = deep-jewel diagonal STRIPES (the old metallic look, darkened for night) + ink edge — NO neon glow, NO shine; the now-line stays the brightest thing (David 2026-06-27)
        card.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.4) + "," + mixHex(D.c, "#160510", 0.4) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; card.style.borderColor = "#160510"; card.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.09),0 3px 0 #160510";
        ...
        card.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.56) + "," + mixHex(D.c, "#160510", 0.56) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; card.style.borderColor = "#160510"; card.style.boxShadow = "0 2px 0 #160510";
      ...
      if (_straddle) { // STRADDLING NOW (David 2026-06-27): the GHOST half (not done) is a standalone fully-rounded bubble; the TRACKED half is CONTINUOUS with the future (no gap at the now-line — the line just shows printing + the battery: bright charged-past, dim future).
        ...
        var _matte = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.56) + "," + mixHex(D.c, "#160510", 0.56) + " 7px," + D.dark + " 7px," + D.dark + " 14px)", _R = "13px";
        ...
        if (_trk) { // TRACKING → the GHOST separates from the active bubble (this is when it breaks off and gets its rounded bottom) — David 2026-06-27
          ...
          if (_hasTrk) { var _segH = (now - _tsm) / 60 * HP, _seg = add(cal, "div", "matchseg"); _seg.style.top = topFor(_tsm) + "px"; _seg.style.height = _segH + "px"; _seg.style.left = "26px"; _seg.style.right = "4px"; _seg.dataset.mn = _tsm; _seg.dataset.dur = (now - _tsm); _seg.style.borderRadius = _R + " " + _R + " 0 0"; _seg.style.borderBottom = "none"; _seg.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.4) + "," + mixHex(D.c, "#160510", 0.4) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; _seg.style.borderColor = "#160510"; _seg.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.09)"; if (_segH >= 15) { var _ssc = add(_seg, "div", "mscn"); _ssc.style.color = D.light; _ssc.innerHTML = tiIcon(b) + ' <span class="cn-t">' + esc(b.title) + '</span> <i class="ti ti-circle-check"></i>'; } else if (_segH >= 7) { var _se = add(_seg, "div", "msemoji"); _se.innerHTML = tiIcon(b); _se.style.cssText = "position:absolute;right:7px;top:50%;transform:translateY(-50%);font-size:11px;line-height:1;color:#fff2f9;"; } } // (2a) TRACKED stretch = bright activity-colour, rounded TOP, SQUARE bottom (flows into the future — no gap); thin sliver → emoji only, no pink
      ...
      if (partial) { // overlay the MATCHED span — a full-width shining segment over both lanes; the rest of the block stays ghost (left) with the drift in the right lane = the split
        ...
        seg.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.4) + "," + mixHex(D.c, "#160510", 0.4) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; seg.style.borderColor = "#160510"; seg.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.09),0 2px 0 #160510";
