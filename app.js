  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (_straddle) { // STRADDLING NOW (David 2026-06-25): the present line SPLITS the block. A FAILED plan is a SPLIT, not a connected bar — the ghost past stays in the PLAN (left) lane so the REAL lane beside it is free for whatever you actually did. Only the TRACKED stretch fuses full-width.
        ...
        card.classList.add("convbar"); card.style.filter = "none"; card.style.opacity = "1"; card.style.background = "none"; card.style.borderColor = _trk ? "transparent" : "#160510"; card.style.boxShadow = _trk ? "none" : "0 3px 0 #160510"; // when tracking, the card is just an invisible holder for the narrow ghost + plan name; the full-width overlays carry the visuals. When NOT doing it, it stays a bordered thin plan-lane bar (David 2026-06-27)
        ...
        var _matte = "repeating-linear-gradient(45deg," + D.light + "," + D.light + " 7px," + D.c + " 7px," + D.c + " 14px)";
        if (_tsm > bs + 0.5) { var _cg = add(card, "div", "convghost"); _cg.style.height = ((_tsm - bs) / 60 * HP / _ch * 100) + "%"; _cg.style.borderRadius = "11px 11px 0 0"; _cg.style.background = mixHex(D.c, "#160510", 0.86); _cg.style.boxShadow = "inset 0 0 0 2px " + mixHex(D.c, "#160510", 0.32); } // past half NOT tracked = narrow ghost BONE in the plan lane; TOP corners rounded like a real bubble (David 2026-06-27)
        if (_trk) { var _fw = add(cal, "div", "futwide"); _fw.style.cssText = "position:absolute;left:26px;right:4px;border:2px solid #160510;border-top:none;box-sizing:border-box;border-radius:0 0 12px 12px;z-index:1;pointer-events:none;filter:saturate(.72) brightness(.72);box-shadow:0 2px 0 #160510;"; _fw.style.top = topFor(now) + "px"; _fw.style.height = Math.max(5, (be - now) / 60 * HP - 4) + "px"; _fw.style.background = _matte; _fw.dataset.mn = now; _fw.dataset.dur = (be - now); } // once you START doing it, the FUTURE half goes FULL-WIDTH like every other future task, bottom corners rounded, no harsh top edge (the pink now-line is the divider) — David 2026-06-27
        else { var _fut = add(card, "div", "convfut"); _fut.style.top = ((now - bs) / 60 * HP / _ch * 100) + "%"; _fut.style.background = _matte; } // not doing it → future half stays a thin plan-lane bar (starts exactly at the now-line)
        if (_trk && now > _tsm + 0.5) { var _segH = Math.max(0, (now - _tsm) / 60 * HP - 2); var _seg = add(cal, "div", "matchseg"); _seg.style.top = topFor(_tsm) + "px"; _seg.style.height = _segH + "px"; _seg.style.left = "26px"; _seg.style.right = "4px"; _seg.dataset.mn = _tsm; _seg.dataset.dur = (now - _tsm); _seg.style.background = "repeating-linear-gradient(45deg," + D.c + "," + D.c + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; _seg.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.18)"; /* NO pink bottom-line — the pink now-line sits right below; it was swamping the thin freshly-played sliver into looking all-pink (David 2026-06-27) */ if (_segH >= 15) { var _ssc = add(_seg, "div", "mscn"); _ssc.style.color = "#fff2f9"; _ssc.innerHTML = tiIcon(b) + ' <span class="cn-t">' + esc(b.title) + '</span> <i class="ti ti-circle-check"></i>'; add(_seg, "div", "shine"); } else if (_segH >= 7) { var _se = add(_seg, "div", "msemoji"); _se.innerHTML = tiIcon(b); _se.style.cssText = "position:absolute;right:7px;top:50%;transform:translateY(-50%);font-size:11px;line-height:1;color:#fff2f9;"; } } // TRACKED stretch = light-blue (activity colour) full-width bar; a thin freshly-played sliver shows just a tiny emoji on the right, the full name appears once it's tall enough
        _convFused = true;
      }
