...
(function () {
  ...
  function init() {
    ...
    setInterval(function () {
      ...
      // per-second PRINTING: between minute re-renders, creep the now-line group + grow the live bubble by the sub-minute fraction so tracking visibly "prints" into the past (GPU transform = smooth; especially visible zoomed in). Resets cleanly on each minute re-render. (David 2026-06-27)
      if (!document.querySelector(".calblk.dragging") && !document.querySelector("#pullBody.zooming")) { var _nl2 = document.querySelector(".day-card.cur .nowline"); if (_nl2) { var _cal2 = _nl2.closest(".cal"); if (_cal2) { var _sH2 = +_cal2.dataset.startH || 6, _eH2 = +_cal2.dataset.endH || 30, _hp2 = (_cal2.offsetHeight - 10) / (_eH2 - _sH2), _dy = new Date().getSeconds() * _hp2 / 3600, _ng = _cal2.querySelectorAll(".nowline, .nowcirc, .nowread, .nowtime"); for (var _gi = 0; _gi < _ng.length; _gi++) _ng[_gi].style.transform = "translateY(" + _dy + "px)"; var _lb = _cal2.querySelector(".calblk.live"); if (_lb && _lb.dataset.baseh != null) _lb.style.height = (parseFloat(_lb.dataset.baseh) + _dy) + "px"; } } }
