...
(function () {
  ...
  function gripHold(ev, card, beginResize) {
    ...
    var sy = ev.clientY, sx = ev.clientX, lastY = sy, scrolling = false;
    function mv(e) { var ady = Math.abs(e.clientY - sy), adx = Math.abs(e.clientX - sx); if (!scrolling && (ady > 22 || adx > 22)) { scrolling = true; if (holdT) { clearTimeout(holdT); holdT = null; } } if (scrolling && scE) { e.preventDefault(); scE.scrollTop -= (e.clientY - lastY); } lastY = e.clientY; }
    function up() { if (holdT) { clearTimeout(holdT); holdT = null; } document.removeEventListener("pointermove", mv); document.removeEventListener("pointerup", up); }
    var holdT = setTimeout(function () { if (scrolling) return; holdT = null; document.removeEventListener("pointermove", mv); document.removeEventListener("pointerup", up); try { if (navigator.vibrate) navigator.vibrate(8); } catch (e) {} beginResize(lastY); }, 270);
  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (_straddle) { // STRADDLING NOW (David 2026-06-25): the present line SPLITS the block. A FAILED plan is a SPLIT, not a connected bar — the ghost past stays in the PLAN (left) lane so the REAL lane beside it is free for whatever you actually did. Only the TRACKED stretch fuses full-width.
        ...
        card.classList.add("convbar"); card.style.filter = "none"; card.style.opacity = "1"; card.style.background = "none"; card.style.boxShadow = "0 3px 0 #160510"; card.style.borderColor = "#160510"; // stays in the plan lane (left) — no full-width, no fusedbar
        var _fut = add(card, "div", "convfut"); _fut.style.top = ((now - bs) / (be - bs) * 100) + "%"; _fut.style.background = "repeating-linear-gradient(45deg," + D.light + "," + D.light + " 7px," + D.c + " 7px," + D.c + " 14px)"; // FUTURE half = matte (still ahead), plan lane
        if (_tsm > bs + 0.5) { var _cg = add(card, "div", "convghost"); _cg.style.height = ((_tsm - bs) / (be - bs) * 100) + "%"; _cg.style.background = mixHex(D.c, "#160510", 0.86); _cg.style.boxShadow = "inset 0 0 0 2px " + mixHex(D.c, "#160510", 0.32); } // past half NOT tracked = ghost BONE in the PLAN lane only — real lane stays empty/your-actual-activity (David 2026-06-25)
        if (_trk && now > _tsm + 0.5) { var _seg = add(cal, "div", "matchseg"); _seg.style.top = topFor(_tsm) + "px"; _seg.style.height = Math.max(5, (now - _tsm) / 60 * HP - 4) + "px"; _seg.style.left = "26px"; _seg.style.right = "4px"; _seg.style.background = "repeating-linear-gradient(45deg," + D.c + "," + D.c + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; _seg.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 #ff5fa8"; var _ssc = add(_seg, "div", "mscn"); _ssc.style.color = "#fff2f9"; _ssc.innerHTML = tiIcon(b) + ' <span class="cn-t">' + esc(b.title) + '</span> <i class="ti ti-circle-check"></i>'; add(_seg, "div", "shine"); } // TRACKED stretch = ONE connected full-width striped (matched) bar across both lanes (planned + executed)
        _convFused = true;
      }
      ...
      card.addEventListener("pointerdown", function (ev) {
        ...
        function mv2(e) {
          ...
          var dy = e.clientY - sy0, dx = e.clientX - sx0; if (!moved && (Math.abs(dy) > 3 || Math.abs(dx) > 3)) { moved = true; if (!snapped) { snapped = true; pushUndo(); } card.classList.add("lift"); card.classList.add("dragging"); } if (moved) { e.preventDefault(); showTrash(); overTrash(e.clientX, e.clientY); dragMin = Math.max(0, Math.min(1740, sm0 + Math.round((dy / HP * 60) / 15) * 15)); card.style.top = topFor(dragMin) + "px"; if (ct0) ct0.textContent = fmt(dragMin) + "–" + fmt(dragMin + (b.mins || 30)); if (_freeDrag) { card.style.transform = "translateX(" + dx + "px) scale(1.04)"; card.style.zIndex = "30"; card.classList.toggle("over-real", dx > 45); card.classList.toggle("over-plan", !!b.done && dx < -45); } else preview(card, dragMin, dragMin + (b.mins || 30)); } // a past bubble isn't lane-locked — it floats under the finger so you can fling it to the bin OR across lanes (David 2026-06-25)
        }
        function up2(e) { var wasMoved = moved, wasScroll = scrolling, trashed = wasMoved && e && overTrash(e.clientX, e.clientY); var dxEnd = e ? e.clientX - sx0 : 0, fling = _freeDrag && wasMoved && !trashed && Math.abs(dxEnd) > 45; card.style.transform = ""; card.style.zIndex = ""; card.classList.remove("over-real"); card.classList.remove("over-plan"); clean();
          ...
          else if (!wasScroll) editBlk(b); } // bin → delete · fling → convert · moved → reorder · tap → edit
    ...
    acts.forEach(function (it) {
      ...
      if (it.kind === "timer") return; // the live activity isn't a growing block — it's the now-line + right-side readout + the pull-up dock (David 2026-06-25)
      ...
      if (it.kind === "log") {
        ...
        card.addEventListener("pointerdown", function (ev) { // hold to rearrange a past activity · drag = scroll · tap = re-label/menu (David 2026-06-24)
          ...
          function up2(ev3) { var wasMoved = moved, wasScroll = scrolling, trashed = wasMoved && ev3 && overTrash(ev3.clientX, ev3.clientY); var dxEnd = ev3 ? ev3.clientX - sx : 0, droppedPlan = _freeDrag && wasMoved && !trashed && dxEnd < -45; card.style.transform = ""; card.style.zIndex = ""; card.classList.remove("over-plan"); clean();
            ...
            else if (wasMoved) { e.time = pad(Math.floor(cur2 / 60)) + ":" + pad(cur2 % 60); reflowLogs(k); save(); renderToday(); } else if (!wasScroll) logEdit(e, k); } // tap a real bubble → full edit menu (type / duration / start / priority), not just rename (David 2026-06-25)
          function cancel() { clean(); }
          document.addEventListener("pointermove", mv2, { passive: false }); document.addEventListener("pointerup", up2); document.addEventListener("pointercancel", cancel);
        });
      } else {
        var t = it.ref, dom = domainOf(t), D = DOM[dom], drift = (dom === "drift"), onp = !drift && onPlanMatch(it, dom);
        card.style.borderColor = "#160510"; card.style.background = drift ? D.c : onp ? ("repeating-linear-gradient(45deg," + D.light + "," + D.light + " 7px," + D.c + " 7px," + D.c + " 14px)") : ("linear-gradient(150deg," + D.light + "," + D.c + ")"); if (onp) { add(card, "div", "shine"); add(card, "div", "foil"); } // on-plan LIVE = SHINING stripes · drift = mauve gradient (David 2026-06-25)
        if (onp) card.classList.add("onplan"); else if (drift) card.classList.add("drift");
    ...
    cal.addEventListener("pointerdown", function (ev) {
      ...
      var planSide = !(lx0 > rect0.width * 0.5 && showNow); // left lane = PLAN (whole width on past days)
      ...
      function up(e) {
        ...
        if (lx0 > rect0.width * 0.5 && showNow) {
    ...
    var _ry = -99; railItems.forEach(function (it) { var y = Math.max(it.y - 8, _ry + 2); if (nowRightBand && y + 16 > nowRightBand[0] && y < nowRightBand[1]) y = nowRightBand[1] + 2; _ry = y + 16; var chip = add(cal, "div", "railchip"); chip.style.top = y + "px"; chip.style.background = it.c; chip.style.color = it.ink; chip.innerHTML = '<i class="ti ' + it.ic + '"></i>'; }); // a chip that would land in the NOW readout's band jumps below it → chips sit above & below now, never under it (David 2026-06-25)
  }
  ...
  function blockEdit(b, k) {
    var B = el("sheetBody"); B.innerHTML = ""; openSheet(); el("sheet").classList.add("edsheet"); pushUndo(); add(B, "div", "sttl", "Edit block");
    ...
    var swb = add(B, "button", "add2 swcol"); swb.style.margin = "8px 0 4px"; var _swD = DOM[domainOf(b)] || DOM.focus; function _paint(D2) { swb.style.setProperty("background", D2.c, "important"); swb.style.setProperty("color", D2.ink, "important"); swb.style.setProperty("border-color", "#160510", "important"); } _paint(_swD); swb.innerHTML = '<i class="ti ' + tiClass(b) + '"></i> switch activity'; swb.onclick = function () { bentoPicker({ title: "Switch to…", onPick: function (x) { b.title = x.title; b.catK = x.catK || null; b.color = x.color || b.color; if (x.domain) b.domain = x.domain; tx.value = x.title; _paint(DOM[domainOf(b)] || DOM.focus); swb.innerHTML = '<i class="ti ' + tiClass(b) + '"></i> switch activity'; reflow(k); save(); renderToday(); } }); }; // the switch button wears the CURRENT activity's colour (David 2026-06-25)
    function _dlbl(m) { return m < 60 ? m + "m" : (m % 60 ? (Math.floor(m / 60) + "h " + (m % 60) + "m") : (m / 60 + "h")); }
    var _rng; function _updRng() { if (_rng) _rng.innerHTML = '<i class="ti ti-clock"></i> begins <b>' + fmt(hm(b.time)) + '</b> · ends <b>' + fmt(hm(b.time) + (b.mins || 30)) + '</b>'; }
    add(B, "div", "lbl", "duration — tap ± to set how long (past or future)"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setD((b.mins || 30) - 5); }; var vv = add(w, "span", "stpv", _dlbl(b.mins || 30)); add(w, "button", "stp", "+").onclick = function () { setD((b.mins || 30) + 5); }; function setD(v) { b.mins = Math.max(5, Math.min(720, v)); vv.textContent = _dlbl(b.mins); if (!blockPast(k, b)) reflow(k); save(); renderToday(); _updRng(); } })();
    _rng = add(B, "div", "rngread"); _updRng(); // begins/ends readout — changing the length OR the start updates it live (David 2026-06-25)
    add(B, "div", "lbl", "start"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setS(hm(b.time) - 5); }; var vv = add(w, "span", "stpv", fmt(hm(b.time))); add(w, "button", "stp", "+").onclick = function () { setS(hm(b.time) + 5); }; function setS(v) { v = Math.max(0, Math.min(1740, v)); b.time = pad(Math.floor(v / 60)) + ":" + pad(v % 60); if (!blockPast(k, b)) reflow(k); save(); renderToday(); vv.textContent = fmt(hm(b.time)); tm.value = b.time; _updRng(); } })();
    add(B, "div", "lbl", "priority"); var c3 = add(B, "div", "pchips"); PRIOS.forEach(function (p) { var x = add(c3, "div", "pchip" + (p.v === (b.prio || 2) ? " on" : ""), p.l); x.onclick = function () { b.prio = p.v; Array.prototype.forEach.call(c3.children, function (n) { n.classList.remove("on"); }); x.classList.add("on"); }; }); // priority shows on EVERY bubble you open, not just future ones (David 2026-06-25)
  ...
  function logEdit(e, k) { // tapping a REAL-lane bubble opens the SAME kind of menu as a plan bubble — type, duration, start, priority (David 2026-06-25)
    var B = el("sheetBody"); B.innerHTML = ""; openSheet(); el("sheet").classList.add("edsheet"); pushUndo(); add(B, "div", "sttl", "Edit activity");
    var frm = add(B, "div", "frm"); var tm = document.createElement("input"); tm.type = "time"; tm.value = e.time; var tx = document.createElement("input"); tx.type = "text"; tx.value = e.title; frm.appendChild(tm); frm.appendChild(tx);
    add(B, "div", "lbl", "type — what this activity is"); var swb = add(B, "button", "add2 swcol"); swb.style.margin = "4px 0"; var _swD = DOM[domainOf(e)] || DOM.focus; function _paint(D2) { swb.style.setProperty("background", D2.c, "important"); swb.style.setProperty("color", D2.ink, "important"); swb.style.setProperty("border-color", "#160510", "important"); } _paint(_swD); swb.innerHTML = '<i class="ti ' + tiClass(e) + '"></i> ' + esc(e.title) + ' — switch'; swb.onclick = function () { bentoPicker({ title: "Switch to…", onPick: function (x) { e.title = x.title; e.catK = x.catK || null; e.color = x.color || e.color; if (x.domain) e.domain = x.domain; tx.value = x.title; _paint(DOM[domainOf(e)] || DOM.focus); reflowLogs(k); save(); renderToday(); swb.innerHTML = '<i class="ti ' + tiClass(e) + '"></i> ' + esc(e.title) + ' — switch'; } }); }; // switch button wears the current activity's colour (David 2026-06-25)
    function _dlbl(m) { return m < 60 ? m + "m" : (m % 60 ? (Math.floor(m / 60) + "h " + (m % 60) + "m") : (m / 60 + "h")); }
    var _rng; function _updRng() { if (_rng) _rng.innerHTML = '<i class="ti ti-clock"></i> begins <b>' + fmt(hm(e.time)) + '</b> · ends <b>' + fmt(hm(e.time) + (e.mins || 15)) + '</b>'; }
    add(B, "div", "lbl", "duration — tap ± to set how long (past or future)"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setD((e.mins || 15) - 5); }; var vv = add(w, "span", "stpv", _dlbl(e.mins || 15)); add(w, "button", "stp", "+").onclick = function () { setD((e.mins || 15) + 5); }; function setD(v) { e.mins = Math.max(5, Math.min(720, v)); vv.textContent = _dlbl(e.mins); reflowLogs(k); save(); renderToday(); _updRng(); } })();
    _rng = add(B, "div", "rngread"); _updRng();
    add(B, "div", "lbl", "start"); (function () { var w = add(B, "div", "stepper"); add(w, "button", "stp", "−").onclick = function () { setS(hm(e.time) - 5); }; var vv = add(w, "span", "stpv", fmt(hm(e.time))); add(w, "button", "stp", "+").onclick = function () { setS(hm(e.time) + 5); }; function setS(v) { v = Math.max(0, Math.min(1740, v)); e.time = pad(Math.floor(v / 60)) + ":" + pad(v % 60); reflowLogs(k); save(); renderToday(); vv.textContent = fmt(hm(e.time)); tm.value = e.time; _updRng(); } })();
    add(B, "div", "lbl", "priority"); var c3 = add(B, "div", "pchips"); PRIOS.forEach(function (p) { var x = add(c3, "div", "pchip" + (p.v === (e.prio || 2) ? " on" : ""), p.l); x.onclick = function () { e.prio = p.v; Array.prototype.forEach.call(c3.children, function (n) { n.classList.remove("on"); }); x.classList.add("on"); }; });
    var row = add(B, "div", "frm");
    add(row, "button", "add", "🗑 Delete").onclick = function () { var a = logs(k); a.splice(a.indexOf(e), 1); save(); closeSheet(); renderAll(); };
    add(B, "button", "done2", "Save").onclick = function () { e.title = tx.value.trim() || e.title; e.time = tm.value; reflowLogs(k); save(); closeSheet(); renderAll(); };
  }
  ...
  function stopTimer(id) { var i = -1; S.timers.forEach(function (t, k) { if (t.id === id) i = k; }); if (i < 0) return; var t = S.timers[i]; if ((Date.now() - t.start) / 1000 < 15) { S.timers.splice(i, 1); save(); renderAll(); toast("⏱ too short — discarded"); return; } var dk = t.dayK || key(new Date(t.start)), mins = Math.max(1, Math.round((Date.now() - t.start) / 60000)), d = new Date(t.start); logs(dk).push({ id: uid(), time: pad(d.getHours()) + ":" + pad(d.getMinutes()), title: t.title, mins: mins, habitId: t.habitId, catK: t.catK, color: t.color }); if (t.habitId) doneMap(dk)[t.habitId] = true; if (isTidy(t)) S.lastTidy = dk; earn(mins, { catK: t.catK }); var opb = onPlanBlockFor(t, dk); if (opb) { opb.done = true; var bonus = Math.max(12, Math.round(mins * 0.4)); earn(bonus, {}); try { celebrate((DOM[domainOf(t)] || DOM.focus).c, bumpStreak()); } catch (e) {} toast("✨ stuck to your plan · +" + bonus + " Spark"); } S.timers.splice(i, 1); save(); renderAll(); } // reward completing a PLANNED activity: light it gold + bonus Spark + a streak (David 2026-06-24 night)
