...
(function () {
  ...
  var LIFESTAGES = [
    { k: "student", l: "Student", ti: "ti-backpack", occ: "student", c: "#36b3f0" }, { k: "parent", l: "Parent", ti: "ti-baby-carriage", occ: "other", c: "#ff5fa0" },
    { k: "founder", l: "Founder", ti: "ti-rocket", occ: "founder", c: "#b07aff" }, { k: "office", l: "9-to-5", ti: "ti-briefcase", occ: "office", c: "#7f9bc4" },
    { k: "freelancer", l: "Freelancer", ti: "ti-device-laptop", occ: "dev", c: "#34d39a" }, { k: "creative", l: "Creative", ti: "ti-palette", occ: "artist", c: "#ffc83d" },
    { k: "caregiver", l: "Caregiver", ti: "ti-heart-handshake", occ: "other", c: "#2ab8c4" }, { k: "figuring", l: "Figuring it out", ti: "ti-compass", occ: "other", c: "#ff8a3a" }
  ];
  ...
  var BUNDLES = [
    { l: "Gym life", ti: "ti-barbell", acts: [["Gym", "move"], ["Run", "move"], ["Stretch", "move"], ["Protein", "nourish"]] },
    { l: "Creative", ti: "ti-palette", acts: [["Draw", "create"], ["Paint", "create"], ["Write", "create"], ["Music", "create"]] },
    { l: "Morning routine", ti: "ti-sun", acts: [["Make the bed", "upkeep"], ["Meditate", "restore"], ["Journal", "restore"], ["Stretch", "move"]] },
    { l: "Study", ti: "ti-book", acts: [["Study", "focus"], ["Read", "play"], ["Research", "focus"]] },
    { l: "Self-care", ti: "ti-heart", acts: [["Meditate", "restore"], ["Bath", "restore"], ["Walk", "move"], ["Journal", "restore"]] }
  ];
  var KRYPTONITE = ["Endless scrolling", "Procrastination", "Hitting snooze", "Overwhelm / freeze", "Perfectionism", "Saying yes to everything", "Junk food", "Doomscrolling", "Comparison", "Distraction"];
  var CONSTRAINTS = ["9–5 job", "Young kids", "Long commute", "Night shifts", "Studying", "Caregiving", "Side hustle", "Pretty flexible"];
  var IDENTITY = [["The Creator", "ti-palette"], ["The Athlete", "ti-run"], ["The Builder", "ti-tools"], ["The Scholar", "ti-book"], ["The Caregiver", "ti-heart-handshake"], ["The Leader", "ti-flag"], ["The Calm one", "ti-wind"], ["The Disciplined one", "ti-target"], ["The Connector", "ti-users"], ["The Explorer", "ti-compass"]];
  var INTENSITY = [["vanilla", "Vanilla", "gentle, no pressure", "ti-leaf"], ["light", "Light", "a little game", "ti-sparkles"], ["game", "Full Game", "go all in", "ti-flame"]];
  function stageSuggest(age) { return (age === "teens" || age === "20s") ? "student" : age === "30s" ? "founder" : (age === "40s" || age === "50s") ? "office" : "figuring"; }
  ...
  var STAGE_EXTRA = {
    ...
    parent: [["Family", "connect"], ["Meal prep", "nourish"], ["Errands", "upkeep"], ["Laundry", "upkeep"], ["Play with kids", "connect"]],
    founder: [["Deep work", "focus"], ["Claude code", "focus"], ["Outreach", "focus"], ["Plan", "focus"]],
    office: [["Deep work", "focus"], ["Meetings", "focus"], ["Email", "focus"]],
    ...
    caregiver: [["Help", "connect"], ["Errands", "upkeep"], ["Journal", "restore"]],
    figuring: [["Journal", "restore"], ["Explore", "play"], ["Study", "focus"]]
  };
  function prefillByStage(stageKey) {
    var by = {}; DOM_ORDER.forEach(function (d) { by[d] = []; });
    for (var d in STAGE_BASE) STAGE_BASE[d].forEach(function (t) { by[d].push(t); });
    (STAGE_EXTRA[stageKey] || STAGE_EXTRA.figuring).forEach(function (a) { var dm = a[1]; if ((by[dm] || (by[dm] = [])).indexOf(a[0]) < 0) by[dm].push(a[0]); });
    return by;
  }
  function onboard() {
    var data = { gender: "", age: "", vibe: "", stage: "", kept: {}, _pref: "", goals: {}, kryptonite: {}, constraints: {}, moreOf: {}, identity: "", intensity: "light", lark: true, up: "7:00", bed: "11:30" };
    var step = 0, STEPS = 13;
    ...
    // dim → bright selection: unselected = darkened tint of its colour, selected = full colour + gold ring (.on)
    ...
    function finish() {
      ...
      var st = LIFESTAGES.filter(function (x) { return x.k === data.stage; })[0] || LIFESTAGES[2];
      P.gender = data.gender; P.age = data.age; P.vibe = data.vibe; P.stage = data.stage; P.occ = st.occ;
      P.goals = keys(data.goals).join(", "); P.kryptonite = keys(data.kryptonite); P.constraints = keys(data.constraints); P.moreOf = keys(data.moreOf); P.identity = data.identity; P.intensity = data.intensity;
      P.lark = data.lark; P.wake = data.up; P.sleep = data.bed; P.set = true;
    ...
    function draw() {
      ...
      body.className = (step === 0 || step === 12) ? "ob-body center" : "ob-body";
      ...
      if (step === 2) { add(body, "div", "ob-q", "A little about you"); add(body, "div", "ob-sb", "helps me suggest the right life stage"); add(body, "div", "ob-lbl", "YOU ARE"); var gr = add(body, "div", "ob-row"); [["f", "she"], ["m", "he"], ["o", "they"], ["", "skip"]].forEach(function (g) { var c = chip(gr, g[1], data.gender === g[0]); c.onclick = function () { data.gender = g[0]; draw(); }; }); add(body, "div", "ob-lbl", "AGE"); var ar = add(body, "div", "ob-row"); ["teens", "20s", "30s", "40s", "50s", "60+"].forEach(function (a) { var c = chip(ar, a, data.age === a); c.onclick = function () { data.age = a; if (!data.stage) data.stage = stageSuggest(a); draw(); }; }); }
      if (step === 3) { add(body, "div", "ob-q", "What's your life like right now?"); add(body, "div", "ob-sb", "I'll set you up like someone in your shoes — change it anytime"); if (!data.stage) data.stage = stageSuggest(data.age); var gr = add(body, "div", "ob-row"); LIFESTAGES.forEach(function (s) { var on = data.stage === s.k, c = chip(gr, '<i class="ti ' + s.ti + '"></i> ' + s.l, on, s.c, "#160510"); c.onclick = function () { data.stage = s.k; data.kept = {}; data._pref = ""; draw(); }; }); }
      if (step === 4) { var stg = LIFESTAGES.filter(function (x) { return x.k === data.stage; })[0] || LIFESTAGES[2]; add(body, "div", "ob-q", "A " + stg.l + "'s usual life — keep what fits ✨"); add(body, "div", "ob-sb", "pre-loaded · untap what's not you · add a pack or extras"); var pf = prefillByStage(data.stage); if (data._pref !== data.stage) { data._pref = data.stage; DOM_ORDER.forEach(function (d) { (pf[d] || []).forEach(function (t) { data.kept[t] = true; }); }); } var grid = add(body, "div", "ob-grid"); DOM_ORDER.forEach(function (d) { var acts = pf[d]; if (!acts || !acts.length) return; var D = DOM[d], cell = add(grid, "div", "ob-cell"); add(cell, "div", "ob-dh", D.l.toUpperCase()).style.color = D.light; var w = add(cell, "div", "ob-row"); acts.forEach(function (t) { var on = data.kept[t]; var c = chip(w, esc(t) + (on ? ' ✓' : ''), on, D.c, D.ink); c.onclick = function () { data.kept[t] = !data.kept[t]; draw(); }; }); }); add(body, "div", "ob-lbl", "✦ ADD A PACK"); var bw = add(body, "div", "ob-row"); BUNDLES.forEach(function (bd) { var c = chip(bw, '<i class="ti ' + bd.ti + '"></i> + ' + bd.l, false, "#9a7cff", "#241548"); c.onclick = function () { bd.acts.forEach(function (a) { data.kept[a[0]] = true; }); draw(); }; }); add(body, "div", "ob-extras-h", "✦ LITTLE EXTRAS"); var ex = add(body, "div", "ob-row"); EXTRAS2.forEach(function (t) { var on = data.kept[t]; var c = chip(ex, esc(t) + (on ? ' ✓' : ''), on, "#8f9fd0", "#16243a"); c.onclick = function () { data.kept[t] = !data.kept[t]; draw(); }; }); }
      if (step === 5) { add(body, "div", "ob-q", "What are you working toward?"); add(body, "div", "ob-sb", "pick a few — fewer is better"); var gr = add(body, "div", "ob-row"); GOAL_SEED.forEach(function (g) { var on = data.goals[g.l], D = DOM[g.d]; var c = chip(gr, '<i class="ti ' + g.ti + '"></i> ' + g.l + (on ? ' ✓' : ''), on, D.c, D.ink); c.onclick = function () { data.goals[g.l] = !data.goals[g.l]; draw(); }; }); }
      if (step === 6) { add(body, "div", "ob-q", "What trips you up?"); add(body, "div", "ob-sb", "your kryptonite — I'll help guard against it"); var gr = add(body, "div", "ob-row"); KRYPTONITE.forEach(function (kr) { var on = data.kryptonite[kr], c = chip(gr, esc(kr) + (on ? ' ✓' : ''), on, "#c4607f", "#3a0f1e"); c.onclick = function () { data.kryptonite[kr] = !data.kryptonite[kr]; draw(); }; }); }
      if (step === 7) { add(body, "div", "ob-q", "What's fixed in your days?"); add(body, "div", "ob-sb", "so plans fit your real life"); var gr = add(body, "div", "ob-row"); CONSTRAINTS.forEach(function (cn) { var on = data.constraints[cn], c = chip(gr, esc(cn) + (on ? ' ✓' : ''), on, "#7f9bc4", "#16243a"); c.onclick = function () { data.constraints[cn] = !data.constraints[cn]; draw(); }; }); }
      if (step === 8) { add(body, "div", "ob-q", "What do you want MORE of?"); add(body, "div", "ob-sb", "I'll weight your days toward these"); var gr = add(body, "div", "ob-row"); DOM_ORDER.forEach(function (d) { if (d === "drift") return; var D = DOM[d], on = data.moreOf[d], c = chip(gr, '<i class="ti ' + D.ti + '"></i> ' + D.l + (on ? ' ✓' : ''), on, D.c, D.ink); c.onclick = function () { data.moreOf[d] = !data.moreOf[d]; draw(); }; }); }
      if (step === 9) { add(body, "div", "ob-q", "Who are you becoming?"); add(body, "div", "ob-sb", "your guardian's north star — pick one"); var gr = add(body, "div", "ob-row"); IDENTITY.forEach(function (id) { var on = data.identity === id[0], c = chip(gr, '<i class="ti ' + id[1] + '"></i> ' + id[0], on, "#b07aff", "#241548"); c.onclick = function () { data.identity = id[0]; draw(); }; }); }
      if (step === 10) { add(body, "div", "ob-q", "How game-y do you want it?"); add(body, "div", "ob-sb", "change it anytime in settings"); var col = add(body, "div", "ob-col"); INTENSITY.forEach(function (it2) { var on = data.intensity === it2[0], c = chip(col, '<i class="ti ' + it2[3] + '"></i> <b>' + it2[1] + '</b> — ' + it2[2], on, "#ffc83d", "#5a3a00"); c.onclick = function () { data.intensity = it2[0]; draw(); }; }); }
      if (step === 11) { add(body, "div", "ob-q", "When are you sharpest?"); add(body, "div", "ob-sb", "so I plan deep work right"); var lr = add(body, "div", "ob-row"); var m = chip(lr, '<i class="ti ti-sun"></i> Morning', data.lark, "#ffc83d", "#5a3a00"); m.onclick = function () { data.lark = true; draw(); }; var n = chip(lr, '<i class="ti ti-moon"></i> Night', !data.lark, "#9a7cff", "#241548"); n.onclick = function () { data.lark = false; draw(); }; add(body, "div", "ob-lbl", "UP AT"); var ur = add(body, "div", "ob-row"); ["6:00", "7:00", "8:00", "9:00"].forEach(function (t) { var c = chip(ur, t, data.up === t); c.onclick = function () { data.up = t; draw(); }; }); add(body, "div", "ob-lbl", "BED AT"); var brh = add(body, "div", "ob-row"); ["10:00", "11:00", "11:30", "12:30"].forEach(function (t) { var c = chip(brh, t, data.bed === t); c.onclick = function () { data.bed = t; draw(); }; }); }
      if (step === 12) { var w = add(body, "div", "ob-world"); w.innerHTML = '<i class="ti ti-sparkles"></i>'; add(body, "div", "ob-q", "Your world is ready ✨"); add(body, "div", "ob-sb", "seeded with your life. let's make today count."); }
