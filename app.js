  ...
  function buildPull() {
    ...
    if (pullZoom === "week") { // horizontal giant-list of weeks: swipe side-to-side; Today recenters (David 2026-06-24)
      ...
      if (curScroll) {
        ...
        else if (_scrollToFocus) { _scrollToFocus = false; var _fnl = (focus === todayK()) ? curScroll.querySelector(".nowline") : null, _ft = curScroll.querySelector('.day-sec[data-dk="' + focus + '"]'); var _fsep = (_ft && _ft.previousElementSibling && _ft.previousElementSibling.classList.contains("day-stacksep")) ? _ft.previousElementSibling : null; requestAnimationFrame(function () { if (_fnl && _fnl.offsetParent !== null) _fnl.scrollIntoView({ block: "center" }); else if (_fsep) curScroll.scrollTop = Math.max(0, _fsep.offsetTop); else if (_ft) curScroll.scrollTop = Math.max(0, _ft.offsetTop - 4); }); } // a horizontal page turn lands the vertical scroll ON the new focus day FROM ITS TOP — its day-label separator at the viewport top, so you see that one day from the beginning, not half-on/half-off the neighbour (David 2026-06-27)
  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (_straddle) { // STRADDLING NOW (David 2026-06-25): the present line SPLITS the block. A FAILED plan is a SPLIT, not a connected bar — the ghost past stays in the PLAN (left) lane so the REAL lane beside it is free for whatever you actually did. Only the TRACKED stretch fuses full-width.
        ...
        var _ch = Math.max(5, (be - bs) / 60 * HP - 4); // the card's ACTUAL drawn height (place() floor/margin) — map the now-split onto THIS, not (now-bs)/(be-bs)%, or the -4px bottom margin pushes the split ~2px above the now-line = the residual "stripes above now" (David 2026-06-27)
        var _fut = add(card, "div", "convfut"); _fut.style.top = ((now - bs) / 60 * HP / _ch * 100) + "%"; _fut.style.background = "repeating-linear-gradient(45deg," + D.light + "," + D.light + " 7px," + D.c + " 7px," + D.c + " 14px)"; // FUTURE half = matte (still ahead), plan lane — starts EXACTLY at the now-line
        if (_tsm > bs + 0.5) { var _cg = add(card, "div", "convghost"); _cg.style.height = ((_tsm - bs) / 60 * HP / _ch * 100) + "%"; _cg.style.background = mixHex(D.c, "#160510", 0.86); _cg.style.boxShadow = "inset 0 0 0 2px " + mixHex(D.c, "#160510", 0.32); } // past half NOT tracked = ghost BONE in the PLAN lane only — real lane stays empty/your-actual-activity (David 2026-06-25)
