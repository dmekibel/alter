...
(function () {
  ...
  function buildPull() {
    ...
    if (!pb._gw) { // physical gestures, wired once: two-finger PINCH = hour-density zoom · one-finger horizontal SWIPE = page to prev/next day (David 2026-06-24)
      pb._gw = 1;
      var ptrs = {}, pVD0 = 0, pHP0 = 0, pHPLast = 0, pMid0 = 0, pScroll0 = 0, pContTop = 0, pAnchorContent = 0, pAnchorSecDk = null, pAnchorFrac = 0, sX = 0, sY = 0, single = false, swOn = false, swPgr = null, swW = 1;
      ...
      pb.addEventListener("pointerdown", function (e) {
        ...
        else if (n === 2) { single = false; swOn = false; _pinching = 1; if (swPgr) { swPgr.style.transition = "transform .15s"; swPgr.style.transform = "translateX(-33.3333%)"; } swPgr = null; var _sc = pb.querySelector(".day-card.cur .day-cardscroll"); pVD0 = Math.max(8, pvdist()); pHP0 = pHPLast = pullHourPx; pMid0 = pmidY(); pScroll0 = _sc ? _sc.scrollTop : 0; pContTop = _sc ? _sc.getBoundingClientRect().top : 0; pAnchorContent = pScroll0 + (pMid0 - pContTop); var _asec = null, _ass = _sc ? _sc.querySelectorAll(".day-sec") : []; for (var _ai = 0; _ai < _ass.length; _ai++) { if (_ass[_ai].offsetTop <= pAnchorContent) _asec = _ass[_ai]; } if (!_asec && _ass.length) _asec = _ass[0]; pAnchorSecDk = _asec ? _asec.dataset.dk : null; pAnchorFrac = (_asec && _asec.offsetHeight > 0) ? Math.max(0, Math.min(1, (pAnchorContent - _asec.offsetTop) / _asec.offsetHeight)) : 0; } // 2 fingers → PINCH+PAN: vertical spread zooms, midpoint pans, both live (iPhone Photos). Anchor to the day-SECTION under your fingers + the fraction within it — fixed-height .day-stacksep headers in the stack break a linear scroll*ratio scale, so scroll is re-derived from the section's fresh offsetTop after relayout (fixes "zoom lands in the wrong area" once there are multiple day-headers — David 2026-06-26)
      });
