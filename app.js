...
(function () {
  ...
  function setStripSel(dk) { var cells = document.querySelectorAll(".pull-weekstrip .pws-day"), found = false; for (var i = 0; i < cells.length; i++) { var m = cells[i].dataset.dk === dk; cells[i].classList.toggle("sel", m); if (m) found = true; } if (!found) { var host = document.querySelector(".pull-weekstrip"); if (host) { host.innerHTML = ""; weekStrip(host, dk); } } } // live-move the week-strip highlight to the day you're heading to; if that day crossed into a NEW week, rebuild the strip on that week so the highlight follows instead of vanishing/sticking on the old week (David 2026-06-27)
