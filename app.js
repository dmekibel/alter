  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (status === "ok" && !partial) {
        card.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.24) + "," + mixHex(D.c, "#160510", 0.24) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; // matched = its own colour, matte-metallic stripes (calmer — no neon glow) (David 2026-06-25)
      ...
      if (partial) { // overlay the MATCHED span — a full-width shining segment over both lanes; the rest of the block stays ghost (left) with the drift in the right lane = the split
        ...
        seg.style.background = "repeating-linear-gradient(45deg," + mixHex(D.c, "#160510", 0.24) + "," + mixHex(D.c, "#160510", 0.24) + " 7px," + D.dark + " 7px," + D.dark + " 14px)"; seg.style.boxShadow = "inset 0 1px 0 rgba(255,255,255,.18),0 2px 0 #160510";
