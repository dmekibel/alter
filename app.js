...
(function () {
  ...
  function relayoutHourPx(nv, anchorY, scrollTop) {
    ...
    if (sc) { if (scrollTop != null && typeof scrollTop === "object") { var _as = sc.querySelector('.day-sec[data-dk="' + scrollTop.dk + '"]'); if (_as) sc.scrollTop = Math.max(0, _as.offsetTop + scrollTop.frac * _as.offsetHeight - scrollTop.fy); } else if (scrollTop != null) sc.scrollTop = Math.max(0, scrollTop); else { var fy = (anchorY == null) ? vh * 0.42 : Math.max(0, Math.min(vh, anchorY - sc.getBoundingClientRect().top)); sc.scrollTop = Math.max(0, (prevTop + fy) * (nv / old) - fy); } }
  }
  ...
  function attachInfinite(sc) { // CONTINUOUS timeline: the day-buffer recenters on whatever day you've scrolled to and the week-strip tracks it — so you just keep scrolling into yesterday/tomorrow with NO edge and NO cut (David 2026-06-26)
    ...
    sc.addEventListener("scroll", function () {
      ...
      if (raf || _infRebuild || _paging || _pinching) return; // never recenter mid-page-turn or mid-pinch (that was the old bounce / pinch snap-back) — David 2026-06-26
  ...
  var _paging = 0, _scrollToFocus = false, _navLock = 0, _pinching = 0; // _pinching suppresses the continuous-scroll recenter while a 2-finger pinch+pan is live (a free pan must not trip a mid-gesture rebuild); _paging suppresses the continuous-scroll recenter during a horizontal page turn; _scrollToFocus jumps the vertical scroll onto the new focus day after a page turn; _navLock blocks the Apple-Music nav-collapse during programmatic scrolls
  ...
  function buildPull() {
    ...
    if (!pb._gw) { // physical gestures, wired once: two-finger PINCH = hour-density zoom · one-finger horizontal SWIPE = page to prev/next day (David 2026-06-24)
      pb._gw = 1;
      var ptrs = {}, pVD0 = 0, pHP0 = 0, pHPLast = 0, pMid0 = 0, pScroll0 = 0, pScrollCur = 0, pContTop = 0, sX = 0, sY = 0, single = false, swOn = false, swPgr = null, swW = 1;
      ...
      pb.addEventListener("pointerdown", function (e) {
        if (e.isPrimary) { ptrs = {}; pVD0 = 0; single = false; swOn = false; swPgr = null; _pinching = 0; if (_zoomRaf) { cancelAnimationFrame(_zoomRaf); _zoomRaf = 0; } pb.classList.remove("zooming"); } // a fresh gesture clears any stale/stuck pointers + zoom-suppress class (David 2026-06-25)
        ...
        else if (n === 2) { single = false; swOn = false; if (swPgr) { swPgr.style.transition = "transform .15s"; swPgr.style.transform = "translateX(-33.3333%)"; } swPgr = null; var _sc = pb.querySelector(".day-card.cur .day-cardscroll"); pVD0 = Math.max(8, pvdist()); pHP0 = pHPLast = pullHourPx; pMid0 = pmidY(); pScroll0 = pScrollCur = _sc ? _sc.scrollTop : 0; pContTop = _sc ? _sc.getBoundingClientRect().top : 0; _pinching = 1; } // 2 fingers → PINCH+PAN, both live & FREE (iPhone Photos): vertical spread zooms about the live finger-midpoint; the midpoint pans as a real accumulating scroll (pScrollCur) with NO fixed anchor → it never clamps to ±1 viewport around today. _pinching suppresses the continuous-recenter so a free pan can't trip a mid-gesture rebuild (David 2026-06-26)
      });
      pb.addEventListener("pointermove", function (e) {
        if (ptrs[e.pointerId]) { ptrs[e.pointerId].x = e.clientX; ptrs[e.pointerId].y = e.clientY; }
        if (!single && pVD0 > 0 && Object.keys(ptrs).length >= 2 && pullZoom === "day") { var vd = Math.max(8, pvdist()); var newHP = Math.max(20, Math.min(300, Math.round(pHP0 * (vd / pVD0)))); var mid = pmidY(); var fyNow = mid - pContTop; var f = newHP / (pHPLast || newHP); pScrollCur = Math.max(0, (pScrollCur + fyNow) * f - fyNow + (pMid0 - mid)); pHPLast = newHP; pMid0 = mid; zoomLive(newHP, null, pScrollCur); e.preventDefault(); return; } // zoom about the live midpoint ((scroll+fy)*f - fy) THEN add the free pan delta (pMid0 - mid) into an accumulating scroll → pan glides freely past day boundaries, never stuck on a fixed anchor (David 2026-06-26)
      ...
      function gend(e) {
        ...
        if (wasPinch && n < 2 && pullZoom === "day") { if (_zoomRaf) { cancelAnimationFrame(_zoomRaf); _zoomRaf = 0; } pVD0 = 0; single = false; _pinching = 0; pullHourPx = pHPLast; zoomCommit(); return; } // settle: crisp re-render at the final zoom, keeping the panned scroll — no bounce (David 2026-06-25)
  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      card.addEventListener("pointerdown", function (ev) {
        ...
        function mv2(e) {
          if (touch && !picked) { var ady = Math.abs(e.clientY - sy0), adx = Math.abs(e.clientX - sx0); if (adx > ady && adx > 10) { if (holdT) { clearTimeout(holdT); holdT = null; } return; } if (!scrolling && (ady > 12 || adx > 12)) { scrolling = true; if (holdT) { clearTimeout(holdT); holdT = null; } } if (scrolling && scE) { e.preventDefault(); scE.scrollTop -= (e.clientY - lastY); lastY = e.clientY; } return; } // horizontal → cancel the pick-up hold and let the day-swipe page · vertical drag = scroll the card · stay within 22px to pick up & move
    ...
    acts.forEach(function (it) {
      ...
      if (it.kind === "log") {
        ...
        card.addEventListener("pointerdown", function (ev) { // hold to rearrange a past activity · drag = scroll · tap = re-label/menu (David 2026-06-24)
          ...
          function clean() { if (holdT) { clearTimeout(holdT); holdT = null; } document.removeEventListener("pointermove", mv2); document.removeEventListener("pointerup", up2); document.removeEventListener("pointercancel", cancel); card.classList.remove("lift"); card.classList.remove("dragging"); hideTrash(); }
          function mv2(ev2) {
            if (touch && !picked) { var ady = Math.abs(ev2.clientY - sy), adx = Math.abs(ev2.clientX - sx); if (adx > ady && adx > 10) { if (holdT) { clearTimeout(holdT); holdT = null; } return; } if (!scrolling && (ady > 12 || adx > 12)) { scrolling = true; if (holdT) { clearTimeout(holdT); holdT = null; } } if (scrolling && scE) { ev2.preventDefault(); scE.scrollTop -= (ev2.clientY - lastY); lastY = ev2.clientY; } return; }
