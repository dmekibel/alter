...
(function () {
  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    (function () {
      ...
      var KF = [[0, [16, 14, 38]], [5, [22, 19, 48]], [6.5, [74, 40, 80]], [8, [30, 36, 78]], [12, [36, 44, 92]], [16, [30, 36, 78]], [18, [58, 38, 80]], [19.5, [86, 42, 82]], [21, [34, 24, 58]], [24, [16, 14, 38]]]; // night deep-indigo (kept) · day = living cool blue (was a dull gray-indigo) · dawn softer rose · dusk a smooth magenta-purple BRIDGE not a muddy red — fixes the day-meh + bad-transition (David 2026-06-27)
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      if (status === "ok" && !partial) { // DONE = dark hollow + a BRIGHT GLOWING domain outline (the reward) + shine — ghost-style but lit (David 2026-06-27)
        card.style.background = mixHex(D.c, "#160510", 0.8); card.style.borderColor = D.c; card.style.boxShadow = "0 0 8px " + D.c + ",inset 0 1px 0 rgba(255,255,255,.1),0 3px 0 #160510";
        add(card, "div", "shine");
      } else if (dark) { // missed/ghost (and the unmatched base of a partial) — domain-tinted-dark hollow + a clear domain OUTLINE
        card.style.background = mixHex(D.c, "#160510", 0.86); card.style.borderColor = mixHex(D.c, "#160510", 0.32); card.style.boxShadow = "none";
      } else { // future = planned/blueprint: dark hollow + a DIM DASHED domain outline, fainter the further ahead (David 2026-06-27)
        card.style.background = mixHex(D.c, "#160510", 0.85); card.style.borderColor = mixHex(D.c, "#160510", 0.42); card.style.borderStyle = "dashed"; card.style.boxShadow = "0 2px 0 #160510";
        var _ahead = (bs - now) / 60; card.style.opacity = String(showNow ? Math.max(0.55, 0.92 - Math.max(0, _ahead) * 0.05) : 0.78);
      }
      if (_straddle) { // STRADDLING NOW (David 2026-06-27): the GHOST half (not done) is a standalone fully-rounded bubble; the TRACKED half is CONTINUOUS with the future (no gap at the now-line — the line just shows printing + the battery: bright charged-past, dim future).
        ...
        var _matte = mixHex(D.c, "#160510", 0.85), _R = "13px";
      ...
      if (partial) { // overlay the MATCHED span — a full-width shining segment over both lanes; the rest of the block stays ghost (left) with the drift in the right lane = the split
        ...
        seg.style.background = mixHex(D.c, "#160510", 0.8); seg.style.borderColor = D.c; seg.style.boxShadow = "0 0 8px " + D.c + ",inset 0 1px 0 rgba(255,255,255,.12),0 2px 0 #160510";
        seg.dataset.mn = _pm.start; seg.dataset.dur = (_pm.end - _pm.start);
        var _sc = add(seg, "div", "mscn"); _sc.style.color = D.light; _sc.innerHTML = (_mh >= 15 ? (tiIcon(b) + ' <span class="cn-t">' + esc(b.title) + '</span> ') : '') + '<i class="ti ti-circle-check"></i>'; add(seg, "div", "shine"); // matched span: icon + name + ✓ (the part you actually did)
      }
    ...
    acts.forEach(function (it) {
      ...
      if (it.kind === "log") {
        ...
        card.style.background = drift ? mixHex(D.c, "#160510", 0.84) : onp ? mixHex(D.c, "#160510", 0.8) : mixHex(D.c, "#160510", 0.84); card.style.borderColor = onp ? D.c : mixHex(D.c, "#160510", 0.34); card.style.boxShadow = (onp ? "0 0 8px " + D.c + "," : "") + "0 3px 0 #160510,0 5px 12px rgba(0,0,0,.4)"; if (onp) { add(card, "div", "shine"); add(card, "div", "foil"); } // ghost-style: dark hollow + domain outline (bright glow when on-plan) (David 2026-06-27)
        ...
        card.style.background = drift ? mixHex(D.c, "#160510", 0.84) : onp ? mixHex(D.c, "#160510", 0.8) : mixHex(D.c, "#160510", 0.84); card.style.borderColor = onp ? D.c : mixHex(D.c, "#160510", 0.34); card.style.boxShadow = (onp ? "0 0 8px " + D.c + "," : "") + "0 3px 0 #160510"; if (onp) { add(card, "div", "shine"); add(card, "div", "foil"); } // ghost-style live: dark hollow + glowing domain outline (David 2026-06-27)
        if (onp) card.classList.add("onplan"); else if (drift) card.classList.add("drift");
