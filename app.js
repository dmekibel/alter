  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      card.addEventListener("pointerdown", function (ev) {
        ...
        var _started = showNow && k === todayK() && sm0 <= now, _floor = (showNow && k === todayK()) ? Math.ceil(now / 15) * 15 : 0; // a block that's already begun (incl. one straddling NOW) is STATIC — can't reorder; a future block can't be dragged back across the present line (David 2026-06-26)
        ...
        function mv2(e) {
          ...
          var dy = e.clientY - sy0, dx = e.clientX - sx0; if (!moved && (Math.abs(dy) > 3 || Math.abs(dx) > 3)) { moved = true; if (!snapped) { snapped = true; pushUndo(); } card.classList.add("lift"); card.classList.add("dragging"); } if (moved) { e.preventDefault(); showTrash(); overTrash(e.clientX, e.clientY); dragMin = Math.max(_floor, Math.min(1740, sm0 + Math.round((dy / HP * 60) / 15) * 15)); card.style.top = topFor(dragMin) + "px"; if (ct0) ct0.textContent = fmt(dragMin) + "–" + fmt(dragMin + (b.mins || 30)); if (_freeDrag) { card.style.transform = "translateX(" + dx + "px) scale(1.04)"; card.style.zIndex = "30"; card.classList.toggle("over-real", dx > 45); card.classList.toggle("over-plan", !!b.done && dx < -45); } else preview(card, dragMin, dragMin + (b.mins || 30)); } // a past bubble isn't lane-locked — it floats under the finger so you can fling it to the bin OR across lanes (David 2026-06-25)
        }
