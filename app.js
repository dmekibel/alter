...
(function () {
  ...
  function buildPull() {
    ...
    if (!pb._gw) { // physical gestures, wired once: two-finger PINCH = hour-density zoom · one-finger horizontal SWIPE = page to prev/next day (David 2026-06-24)
      pb._gw = 1;
      var ptrs = {}, pVD0 = 0, pHP0 = 0, pHPLast = 0, pMid0 = 0, pScroll0 = 0, pContTop = 0, pAnchorContent = 0, pAnchorSecDk = null, pAnchorMin = 0, pAnchorStartH = 6, pAnchorCalOff = 0, sX = 0, sY = 0, single = false, swOn = false, swPgr = null, swW = 1;
  ...
  function calendarView(L, k, showNow, noHead) {
    ...
    acts.forEach(function (it) {
      ...
      var cardH;
      if (it.kind === "timer") { cardH = Math.max(5, (it.e - it.s) / 60 * HP - 4); card.style.top = topFor(it.s) + "px"; card.style.height = cardH + "px"; card.dataset.mn = it.s; card.dataset.dur = (it.e - it.s); liveBottom = Math.max(liveBottom, topFor(it.s) + cardH); } // LIVE bubble PRINTS into the past: grows from its start down to the now-line as time passes, starting as a thin sliver (too small for text → the now-line readout covers the "what" until it's tall enough to switch to on-bubble text) — David 2026-06-27
      ...
      if (it.kind === "log") {
        ...
        card.addEventListener("pointerdown", function (ev) { // hold to rearrange a past activity · drag = scroll · tap = re-label/menu (David 2026-06-24)
          ...
          if (touch) holdT = setTimeout(function () { if (scrolling || card.dataset.gate === "menu" || _pinching) return; picked = true; holdT = null; card.classList.add("lift"); card.classList.add("dragging"); try { if (navigator.vibrate) navigator.vibrate(9); } catch (e2) {} }, 280);
        ...
        if (onp) card.classList.add("onplan"); else if (drift) card.classList.add("drift");
        var cn = add(card, "div", "cn"); cn.style.color = D.ink; cn.innerHTML = '<i class="ti ti-player-play-filled"></i> <span class="cn-t">' + esc(t.title) + '</span> · <span class="live-elapsed" data-tid="' + t.id + '">' + elapsedStr(t) + '</span>';
