    ...
    var _bsorted = bls.sort(function (a, b) { return hm(a.time) - hm(b.time); }); _bsorted.forEach(function (b, _bi) {
      ...
      card.addEventListener("pointerdown", function (ev) {
        ...
        function mv2(e) {
          ...
          var dy = e.clientY - sy0, dx = e.clientX - sx0; if (!moved && (Math.abs(dy) > 3 || Math.abs(dx) > 3)) { moved = true; if (!snapped) { snapped = true; pushUndo(); } card.classList.add("lift"); card.classList.add("dragging"); } if (moved) { e.preventDefault(); showTrash(); overTrash(e.clientX, e.clientY); dragMin = Math.max(0, Math.min(1740, sm0 + Math.round((dy / HP * 60) / 15) * 15)); card.style.top = topFor(dragMin) + "px"; if (ct0) ct0.textContent = fmt(dragMin) + "–" + fmt(dragMin + (b.mins || 30)); if (_freeDrag) { card.style.transform = "translateX(" + dx + "px) scale(1.04)"; card.style.zIndex = "30"; card.classList.toggle("over-real", overReal(e.clientX)); } else preview(card, dragMin, dragMin + (b.mins || 30)); } // a past bubble isn't lane-locked — it floats under the finger so you can fling it to the bin OR across to the real lane (David 2026-06-25)
        }
        function up2(e) { var wasMoved = moved, wasScroll = scrolling, trashed = wasMoved && e && overTrash(e.clientX, e.clientY); var droppedReal = wasMoved && !trashed && _freeDrag && e && overReal(e.clientX); card.style.transform = ""; card.style.zIndex = ""; card.classList.remove("over-real"); clean();
          ...
          else if (droppedReal) { b.time = pad(Math.floor(dragMin / 60)) + ":" + pad(dragMin % 60); b.done = true; reflow(k); save(); renderToday(); try { if (navigator.vibrate) navigator.vibrate(12); } catch (e2) {} toast("✓ you did it — counted as done"); } // dragged from PLAN across into REAL = "I actually succeeded with this plan" → fused full bar (David 2026-06-25)
          else if (wasMoved) { b.time = pad(Math.floor(dragMin / 60)) + ":" + pad(dragMin % 60); reflow(k); save(); renderToday(); }
          else if (!wasScroll) editBlk(b); } // bin → delete · real lane → succeeded · moved → reorder · tap → edit
      ...
      if (it.kind === "log") {
        ...
        card.addEventListener("pointerdown", function (ev) { // hold to rearrange a past activity · drag = scroll · tap = re-label/menu (David 2026-06-24)
          ...
          function mv2(ev2) {
            ...
            if (card.dataset.gate === "menu") return; // too tiny to move — only its tap-menu acts
            var dy = ev2.clientY - sy, dx = ev2.clientX - sx; if (!moved && (Math.abs(dy) > 3 || Math.abs(dx) > 3)) { moved = true; if (!snapped) { snapped = true; pushUndo(); } card.classList.add("lift"); card.classList.add("dragging"); if (_freeDrag) _ga = gapAdj(); } if (moved) { ev2.preventDefault(); showTrash(); overTrash(ev2.clientX, ev2.clientY); cur2 = Math.max(0, Math.min(nowMin() - dur, sm0 + Math.round((dy / HP * 60) / 5) * 5)); var tpx = topFor(cur2); card.style.top = tpx + "px"; if (_freeDrag) { card.style.transform = "translateX(" + dx + "px) scale(1.04)"; card.style.zIndex = "30"; card.classList.toggle("over-plan", overPlan(ev2.clientX)); if (_ga) _ga(tpx, tpx + (parseFloat(card.style.height) || 0)); } }
          }
          function up2(ev3) { var wasMoved = moved, wasScroll = scrolling, trashed = wasMoved && ev3 && overTrash(ev3.clientX, ev3.clientY); var droppedPlan = wasMoved && !trashed && _freeDrag && ev3 && overPlan(ev3.clientX); card.style.transform = ""; card.style.zIndex = ""; card.classList.remove("over-plan"); clean();
            ...
            else if (droppedPlan) { e.time = pad(Math.floor(cur2 / 60)) + ":" + pad(cur2 % 60); blocks(k).push({ id: uid(), time: e.time, mins: e.mins || dur, title: e.title, domain: domainOf(e), color: e.color || (DOM[domainOf(e)] || DOM.focus).c, catK: e.catK || null, prio: 2, done: true }); reflow(k); save(); renderToday(); try { if (navigator.vibrate) navigator.vibrate(12); } catch (e2) {} toast("✓ counted as a kept plan"); } // dragged from REAL across into PLAN = "this WAS the plan, and I did it" → a matched plan block (fuses with the log) (David 2026-06-25)
            else if (wasMoved) { e.time = pad(Math.floor(cur2 / 60)) + ":" + pad(cur2 % 60); reflowLogs(k); save(); renderToday(); } else if (!wasScroll) relabel(); }
          function cancel() { clean(); }
          document.addEventListener("pointermove", mv2, { passive: false }); document.addEventListener("pointerup", up2); document.addEventListener("pointercancel", cancel);
        });
      } else {
