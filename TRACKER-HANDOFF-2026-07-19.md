...
**Audit:** 105 PASS / 1 pre-existing environmental FAIL (deep-rows-quiet: IO deadmen on throttled preview boot; reproduced identically on pristine tree — proven not-a-regression).
**ONE move — David:** fresh.html (v1407): ch7/16/26/34 tops + drift, ray stones after stopping, rail top vs gems, full-height slide.
**ONE move — Claude:** the night-face hcAwayHide ring skip (KNOWN since v1402) remains the one open thread.

## STATUS UPDATE (2026-08-28 (20), the arrival that never fired + rail round 2): v1402 → v1405
**v1402** — David: strip/board SOMETIMES missing after arrivals, both directions. Reproduced headlessly (7-sequence harness): park ON home with the finger down, lift → hcMaybeIn refused (_wTouch), its only re-schedule rides scroll events a parked column stops firing, wMaybeSnap returns without springing (already on target), _hcState stuck out* also gated the at-home hcReset. Fix: wTouch(false) queues hcMaybeIn at 160ms; the at-home reset clears a stale away-flag. 7/7 end-state pass. Puck hides by 60% of the way home (was 66-89%). KNOWN: hcAwayHide skips the night ring (tfNightPulse's opacity defeats the visible-check) — rest circle can still show while parked away at night; open.
**v1405** — David's five rail verdicts: top now 42dvh via --jr-top (÷ --tfscale; 168px superseded), invisible at rest (appears on scroll, 900ms decay to nothing), dot z-under-pill + arithmetic blanking, smooth continuous slide (detents dead, release = where the finger left it), landing chip deleted. TEXTURES: .jl-l-tex (mDrift, 35 gate cards) + stone rotating layers now bleed past their boxes (transparent border + border-box clip; background-origin keeps interior BIT-identical; background-position animation rejected because ch10/27's checkerboard IS an inline background-position). HONEST: the gap does NOT reproduce in Chrome (pixel-probed both mDrift extremes) — structural insurance for a likely iOS compositing artifact; if it survives on device, the cause is elsewhere — recording of the one card, no guessing. designAudit 102/102 incl. 5 new gates.
**ONE move — David:** fresh.html (v1405): climb, scroll-wake the rail, slide it; check ch7/ch34 cards + stones.
**ONE move — Claude:** night-face hcAwayHide ring skip (the KNOWN above) next round; then the saga rests.

## STATUS UPDATE (2026-08-28 (19), three device verdicts + THE RAIL): v1400 → v1401
**v1400**, David's three same-night verdicts: (1) the deck tiles NEVER leave toward the tools (v1399's both-directions call reverted — up-exit/down-arrival only, hcAwayHide journey-side only); (2) a gentle up-swipe from home commits UP — wSnapIntent's 45% clearance rule (pre-touch-velocity era) was overruling measured direction; now an explicit-up mirror of the explicit-down check, 45% kept as the direction-unknown tie-break. Verified via DEV.wIntent, 13 cases incl. the one-zone law. Comment corrected: `_wStartTop > hy + 40` = a TOOLS-side start test, not sky. (3) the night ring stays dark through transitions — its rest look IS `tfNightPulse` (stylesheet animation) and the cascade's inline animation was clobbering it for the whole flight; pulse phase-shifted to resume at .9 (same range/period/look), the existing hcFlush animationend handoff documented.
**v1401 — THE JOURNEY RAIL SHIPPED** (David's `Journey Rail.dc.html`, checklist port; pull + spec at `_design-sync/journey-rail-2026-08-28/`): the pill and nothing else — 10x48 hsl(334 100% 65%), rest .31/awake .86/grabbed 13x52+chip, rail right 7/top 168/bottom 191. Rides the one scroll listener (jrScrub off wScrub), zero layout reads, detents from the line's own geometry (wAnchorsDirty-invalidated). Drag per Call 2: 44px zone armed-while-awake, one _wGen++ at grab, _wSelfW-marked writes, detent snap, _wNoSnap (time-bounded) keeps the magnet off the finger, touch-action:none on the zone only. Chip = book line + kicker-hue number tile + name; dot rides .jl-today. NOT built per David's own sliders: return chip (off), dim, haptics (no iOS API). 3 new designAudit gates; 99/99 in preview.
**OPEN for David:** BOOK TWO chip color = the rail frame's #7fd4e0, books one/three keep the line's accents — lighten all three? · rail drag feel + all v1400 choreography DEVICE-UNTESTED.
**ONE move — David:** fresh.html (v1401): one pass — transitions (tiles/gentle-up/night ring), then climb and grab the rail.
**ONE move — Claude:** close the saga in memory if his pass is clean; any miss → 📈 trace first, never theory.

## STATUS UPDATE (2026-08-28 (18), THE TRACE PAYS OFF THREE TIMES): v1397 → v1399 — ENGINE SMOOTH ON DEVICE
David's verdict on v1398: **"the flickering, the jitter is basically completely gone."** Three trace pastes (no videos needed) found three stacked diseases:
**v1397** — the trace caught wSpring/wScrollTo STACKING: shared `_wStop`/`_wAnim` meant every engine start revived older tick loops (two writers 220px apart alternating frames; later three in 2ms) and the spring's tail crept 3.3s sub-pixel, holding the door open. Fix: `_wGen` generation token (one engine owns the column; elders retire without touching shared flags), 1500ms hard landing cap, and hcScrub's mid-glide home arrival gated on the engine's declared `_wTarget` (a wobble frame can no longer fake a homecoming). Also: trace header self-identifies its mode (trace 1 had silently run on mode 13 — fix OFF — only `ext 0` + "no kills" exposed it).
**v1398** — the trace showed iOS starving scroll events during fast drags (a 134px flick = TWO events; `_wV` -0.14 while the finger's true release was -1.10), so his up-flick was answered with a home snap and springs seeded near zero (the mush). Fix: always-on passive touchmove velocity tracker; on release the finger's velocity OUTVOTES the event-derived one (one assignment corrects catch, pole pick, seed, extrapolation, cascade pace), with a 160ms anti-dilution lock. Trace gained `tv` rows + commits labeled `via catch`/`via settle`.
**v1399** — remaining beef was choreography: the rest circle visible-then-blinking on return (minute-tick rebuilds board nodes VISIBLE while parked away — the stale-shelf bug of 2026-08-15, home-board edition → `hcAwayHide`), and "the four tools on the bottom already there": they are `#tfHeroRow` (HOME zone — the 2c dedupe removes them from the shelf), owned by NO cascade → `tcPeek()` folds them with the board on both exits and arrives them left-to-right 45ms-staggered one beat after Planner. designAudit gate ATTEMPTED/BLOCKED (preview state-locked on staged night face); change is transient animation/opacity only — David's 📐 can confirm 12/12 on device.
**KNOWN**: screen-recording itself starves the browser (events 700ms late in the recorded half of his trace) — jitter WHILE recording is the recorder, not the app. Judge by eye; trace for the rest.
**ONE move — David:** fresh.html (v1399): journey→home and tools→home. The circle must never be seen before its cascade; the four tools sweep left-to-right after Planner. If any cascade still reads wrong: 📈 trace → paste.
**ONE move — Claude:** if choreography passes, this saga closes; next up is the journey rail once he designs it in Claude Design (PROMPT-journey-rail.md delivered).
