...
- **Home H-C slice (v1134):** idle-home tools now a static 2x4 grid (`.tf-toolgrid`) = 7 most-worn STACK_TOOLS + a "More" door → openToolbox, replacing the scroller+dots. Matches David's What-now screenshot. Grid CSS verified (4-col, wraps 2 rows, square tiles) but the idle face itself is PREVIEW-UNCONFIRMED (couldn't force past night/claim gates headlessly). DEFERRED to own slice (regression-zone, spec split law): S4 Extend-when-ending chip + S3 tracking-tool gating (both live in the S3 cockpit — don't touch blind). S5 gap-return already = existing `claim` state.
