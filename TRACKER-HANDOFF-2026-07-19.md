...
**Still queued (next fresh Opus session), David's pick order:**
1. **Toolbox overlay bug** — the category list gets covered; needs David on-phone to say which view + what covers what (blind fix risks a wrong guess in the cockpit tool-stage).
2. **Player unification (#1) + more breathing stacks (#2 cont.)** — merge the breath overlay into the composed timelinePlayer so every tool shares one story-bar + transport, center swaps. Medium/high, regression-adjacent. The ladder `flow` engine is a natural building block.
