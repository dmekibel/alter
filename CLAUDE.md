...
6. **THE TOKEN-SHEET GATE (David 2026-07-30, born from the picker pass that guessed).** Before ANY build or spec that touches a designed surface's paint or layout, PULL THE LIVE SOURCE from the Claude Design project (DesignSync: `tokens/colors.css` + `tokens/effects.css` + the specific component `.jsx` behind the surface, project "Alter Design System"). The spec hands the builder EXACT quoted values (hex, px, radius, shadow, gradient); a builder never receives an eyeballed value, a remembered token, or a value re-used from an earlier session's pull (tokens drift). Screenshots are for STRUCTURE and for catching conflicts, never for sampling. Where David's artifact visibly contradicts an older component file, the artifact wins per this law, and the conflict is named to him in one line. Known trap: `get_file` caps at 256KiB and big `.dc.html` tails vanish silently. The pass this gate exists to prevent: 2026-07-30, ignition stripes built at 45deg/9px when the token is 115deg/13-28px white .30, folder shells washed into the wrong night base, and `#160510` printed on hues where `--on-hue #2a1730` is the law.
7. **THE FRAME-WINS ORDER (David 2026-07-30, born from the v1242 rejection).** When sources disagree, the order is absolute: David's WORDS in chat, then his rendered FRAME exports, then DS tokens/component files, then any existing build. The token sheet supplies values only where the frame confirms them; when a frame pixel visibly contradicts a token or component (stripe direction, shell wash), sample the frame export directly (legal per law 4) and use that. Traps this order exists to kill: (a) a mockup frame CROPS at its edges; a shape cut by the frame bottom is a cropped full element, never a designed "peek" (the drawer saga: two cropped folder tops were built into a collapsible drawer nobody designed); (b) selection chrome REPLACES resting chrome unless the frame shows both together (the pink ring must delete the lip shadow, not stack over it); (c) verifying a build against your own spec instead of against the frame ships a consistent WRONG build. Mandatory final gate before any design ship: screenshot every changed state and diff it against the frame ELEMENT BY ELEMENT: every fill, border, shadow thickness, gradient direction, text color, and proportion. David's nine-point 2026-07-30 correction list is the permanent seed checklist. **A SCROLL LANDING IS A STATE (2026-08-14, born from the v1267 tools-landing miss):** on any scrolling surface with snap points, "every changed state" includes each REST POSITION — screenshot the column parked at each landing scrollTop and diff those against the frames too; a build whose elements all pass can still park wrong. And a deviation from a design file's runtime mechanism (a measured pad, a computed inset) must be verified against the OUTCOME that mechanism produces, measured in the preview — never argued away from the app's existing code.

**THE ONE PROCEDURE: `_dev/DESIGN-PORT-CHECKLIST.md` binds laws 6-8 into a single ordered checklist (pull-gate → run the prototype → computed-value extraction → scoped Opus build → 1:1 position diff at the frame's own geometry (2c = 402x874, David's device) → gates → landing screenshots → ship). Read it BEFORE any design port; no step is optional.**
8. **THE PROTOTYPE-IS-THE-SPEC LAW (David 2026-08-14, born from the tools-landing saga — "in theory claude design connection should have worked").** A `.dc.html` design bundle is a RUNNING PROGRAM, not a document. Before porting any behavioral design (scroll, snap, cascade, any runtime-measured layout): (a) OPEN the design file in the browser pane (it's servable from the repo), (b) drive it to every snap/rest state, (c) extract each landing's geometry NUMERICALLY (absolute offsets of the key elements at each landing scrollTop, gaps, measured paddings) — those numbers are the acceptance targets the build must hit, written into the spec and, where locked, into designAudit gates. Reading the reference code and "interpreting" it is FORBIDDEN as a substitute: v1267/v1271 both shipped wrong because a mechanism (`_measurePad`) was reasoned about instead of watched — the running prototype would have shown the deck parking at the top of the tools screen in one scroll. David recording a screen video of the design is damage control for skipping this step, never the procedure.
...
- **Design-audit gate (2026-07-22):** any change touching the HOME surface's paint/layout must run `DEV.designAudit()` in the preview (idle home, tuner vars + `alter_tuner` localStorage cleared) and PASS 12/12 before shipping. It mechanically checks the locked board numbers (circle 64vw, strip→circle gap, ring rim, calm bloom, grid 70vw, door 18x80/0-border/exact fills, raw tile hexes, plain text lines). A FAIL = do not ship. Born after three fidelity misses in one night (David: "you need a better self-auditing system... I don't want to be the one telling you"). Extend the audit whenever a new board number is locked; the in-app dev "Design tuner" is where David sets those numbers on real pixels.
...
0. **NO BUILDING ON FABLE (David 2026-07-18, supersedes the softer 2026-07-02 rule).** Fable sessions = thinking, planning, design synthesis, brainstorming ONLY. ALL building happens on Opus, including regression-zone work, unless David SPECIFICALLY says "build with Fable" in that session. If David asks for a build while on Fable: produce the spec here, then **RUN IT FROM THIS SESSION as an Opus subagent** (Agent tool, `model: "opus"`, SOLO/no-fanout, no-commit) — Fable orchestrates, adversarially audits the result against the spec, ships. NEVER end the session telling David to open an Opus session himself (David 2026-07-27: "do it all with agents… don't waste time telling me to open new sessions" — supersedes the old open-a-session mechanism; the law itself stands: Fable's own context never edits app.js). Same for chat visuals/widget mockups (David 2026-07-18): Fable never renders widgets (credit burn); Fable writes the design brief + beauty/cleverness bar, OPUS renders the widgets in chat and builds. I still own the routing call and say so before any sizable task; route subagent/workflow work to cheaper models via per-agent overrides regardless of session model.
